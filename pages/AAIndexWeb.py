...
def app():
    st.title("Descriptor: AAindex Table")
    ...
    with st.form(key='form'):
        ...
        table = path + 'files/AAindex.csv'
        submit = st.form_submit_button(label='Submit')

    if finput and label_dataset and submit:
        try:
            byte_str = finput.read()
            finput = byte_str.decode('UTF-8')
            my_bar = st.progress(20)
            st.success("Extracting Features!!!")
            foutput = mapping(finput, label_dataset, table)
