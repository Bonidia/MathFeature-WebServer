...
def app():
    st.title("Descriptor: Xmer k-Spaced Ymer composition frequency")
    ...
    with st.form(key='form'):
        ...
        k = st.number_input('Frequency of k-gap, e.g., 1, 2, 3, 4...', min_value=1, step=1)
        before = st.number_input('Number before k-gap, e.g., 1, 2)', min_value=1, step=1)
        after = st.number_input('Number after k-gap, e.g., 1, 2)', min_value=1, step=1)
        labelDataset = st.text_input('Dataset Label, e.g., lncRNA, mRNA, sncRNA')
        seq = st.selectbox('Type of sequence', ['DNA', 'RNA', 'Protein'])
        submit = st.form_submit_button(label='Submit')

    if finput and k and before and after and labelDataset and seq and submit:
        try:
            ...
            if seq == 'DNA':
                ...
                caracteres = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I',
                              'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S',
                              'T', 'V', 'W', 'Y']

            # st.text(caracteres)
            foutput = kgap(finput, labelDataset, k, before, after, caracteres)
