...
def app():
    ...
    with st.form(key='form'):
        ...
        q = st.number_input('Tsallis - entropic parameter q', min_value=0.1, step=0.1)
        submit = st.form_submit_button(label='Submit')

    if finput and ksize and label_dataset and q and submit:
        try:
            ...
            if q == 1.0:
                q = 1.0 + 0.1
            foutput = entropy_equation(finput, ksize, label_dataset, float(q))
