...
def app():
    st.title("Descriptor: Shannon or Tsallis (q = 2)")
    ...
    with st.form(key='form'):
        ...
        # var = st.text_input('Output File, e.g., dataset, test', value='dataset')
    ...
    if finput and ksize and label_dataset and e and submit:
        try:
            byte_str = finput.read()
            finput = byte_str.decode('UTF-8')
            my_bar = st.progress(20)
            st.success("Extracting Features!!!")
            foutput = entropy_equation(finput, ksize, label_dataset, e)
