...
def app():
    st.title("Descriptor: Numerical Mapping Approach")
    ...
    with st.form(key='form'):
        ...
        tech = st.selectbox('Approach', ['Binary', 'Z-curve', 'Real',
                                         'Integer', 'EIIP', 'Complex Number',
                                         'Atomic Number'])
        padd = st.selectbox('Padding, if yes, this method will generate a vector '
                            'with the size of the largest sequence.', ['Yes', 'No'])
    ...
    if submit:
        try:
            ...
            if tech == 'Binary':
                foutput = binary_mapping(finput, label_dataset, padd)
            elif tech == 'Z-curve':
                foutput = zcurve_mapping(finput, label_dataset, padd)
            elif tech == 'Real':
                foutput = real_mapping(finput, label_dataset, padd)
            elif tech == 'Integer':
                foutput = integer_mapping(finput, label_dataset, padd)
            elif tech == 'EIIP':
                foutput = eiip_mapping(finput, label_dataset, padd)
            elif tech == 'Complex Number':
                foutput = complex_number(finput, label_dataset, padd)
            elif tech == 'Atomic Number':
                foutput = atomic_number(finput, label_dataset, padd)
