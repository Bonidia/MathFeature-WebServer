...
from ComplexNetworksClass import *
...
def app():
    st.title("Descriptor: Complex Networks (with threshold)")
    ...
    with st.form(key='form'):
        ...
        ksize = st.number_input('k size, E.g., 2, 3 (default = codon), 4 ...', min_value=1, step=1)
        label_dataset = st.text_input('Dataset Label, e.g., lncRNA, mRNA, sncRNA')
        t = st.number_input('Threshold size, E.g., 2, 3 (default = 10)', min_value=1, step=1)
        submit = st.form_submit_button(label='Submit')

    if finput and ksize and label_dataset and t and submit:
        try:
            byte_str = finput.read()
            finput = byte_str.decode('UTF-8')
            my_bar = st.progress(20)
            st.success("Extracting Features!!!")
            foutput = complex_network(finput, ksize, label_dataset, t + 1)
