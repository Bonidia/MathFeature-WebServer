...
def pseknc_record(foutput, listy, name_seq, label_dataset):
    i = 0
    z = 1
    listz = ''
...
def header_pseknc(foutput, perms_list):
    file = open(foutput, 'a')
...
def header_pseknc2(foutput, perms_list):
    file = open(foutput, 'a')
    file.write('%s,' % ('nameseq'))
    i = 0
    # print (permsList)
...
def generate_permutations(chars, allowed_chars):
    status = []
...
def simplePseKNC(finput, foutput, k, label_dataset, allowed_chars):
    listy = ''
    # Need to generate all possible oligonucleotides
    olinucz = generate_permutations(k, allowed_chars)
    header_pseknc2(foutput, olinucz)
...
def pseKNCHelper(seq, olinucs, props, Type, k, j, w, sup_info):
    gamma = len(props)
    freqs = []
    seqq = sepSequence(seq, k)
...
def pseKNC(finput, foutput, label_dataset, props, Type, k, j, w, xp):
    j = float(j)
    gamma = len(props)
    ...
    for seq_record in SeqIO.parse(io.StringIO(finput), "fasta"):
        ...
        name_seq = seq_record.name	
        if Type == 1:
            listy = pseKNCHelper(seq, olinucs, props, Type, k, j, w, sup_info)
        else:
            listy = pseKNCHelper(seq, olinucs, props, Type, k, j*gamma, w, sup_info)
        if aux == 0:
            header_pseknc(foutput, listy)
            aux = 1
        pseknc_record(foutput, listy, name_seq, label_dataset)
    return


def parameters(xp, x, t, k, j, w, s, finput, label_dataset, seq, allowed_chars):
    ...
    if s == 1:
        simplePseKNC(inputFile, outputFile, int(kay), label_dataset, allowed_chars)
    else:
        if kay == 2 or kay == 3:
            kay = int(kay)
        else:
            ...
            sys.exit()
        pseKNC(finput, foutput, label_dataset, props, Type, kay, lam, weight, xp)
    return foutput
...
if __name__ == "__main__":
    ...
    parameters(xp, x, t, k, j, w, s, finput, foutput, seq)
#############################################################################