...
def headerNAC(foutput, k, caracteres):
    listTotal = []
    file = open(foutput, 'a')
...
def file_record(foutput, name_seq, probabilities, labelDataset):
    file = open(foutput, 'a')
...
def nacSeq(finput, k, caracteres, labelDataset):
    ...
    headerNAC(foutput, k, caracteres)
    for seq_record in SeqIO.parse(io.StringIO(finput), "fasta"):
        ...
        file_record(foutput, name_seq, probabilities, labelDataset)
    return foutput
...
def header_kmer(foutput, ksize, caracteres):
    listTotal = []
    file = open(foutput, 'a')
    file.write('%s,' % 'nameseq')
...
# k-mer, RCKmer
def findKmers(finput, ksize, caracteres, labelDataset):
    ...
    header_kmer(foutput, ksize, caracteres)
    for seq_record in SeqIO.parse(io.StringIO(finput), "fasta"):
        seq = seq_record.seq
        seq = seq.upper()
        name_seq = seq_record.name
...
def kgap_record(foutput, name_seq, number_kmers, labelDataset):
    file = open(foutput, 'a')
    file.write('%s,' % (name_seq))
...
def seqKGAP(finput, caracteres, labelDataset, window, ksize, step):
    ...
    for seq_record in SeqIO.parse(io.StringIO(finput), "fasta"):
        ...
        kgap_record(foutput, name_seq, number_kmers, labelDataset)
    return foutput
