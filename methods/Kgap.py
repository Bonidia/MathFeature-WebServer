...
def kgap_record(foutput, number_kmers, name_seq, labelDataset):
    file = open(foutput, 'a')
    file.write('%s,' % name_seq)
    for x in number_kmers:
        # print (x)
        file.write('%s,' % (str(x[1])))
    file.write(str(labelDataset))
    file.write('\n')
...
def header_kgap(foutput, permsList):
    file = open(foutput, 'a')
...
def compare(chunks, k, label, before):
    table = {}
    # total = len(chunks)
    for i in label:
        count = 0
...
def kgap(finput, labelDataset, k, before, after, caracteres):
    ...
    for seq_record in SeqIO.parse(io.StringIO(finput), 'fasta'):
        ...
        # print(chunks)
        table = compare(chunks, k, label, before)
        for key, value in table.items():
            number_kmers.append([str(key), value])
        kgap_record(foutput, number_kmers, name_seq, labelDataset)
    return foutput
...
if __name__ == "__main__":
    ...
    if seq == 1:
        caracteres = ["A", "C", "G", "T"]
        kgap(finput, labelDataset, k, before, after, caracteres)
    elif seq == 2:
        caracteres = ["A", "C", "G", "U"]
        kgap(finput, labelDataset, k, before, after, caracteres)
    elif seq == 3:
