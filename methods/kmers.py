...
def perms(foutput, ksize, caracteres):
    listTotal = []
    file = open(foutput, 'a')
...
def fileRecord(foutput, name_seq, probabilities, labelDataset):
    file = open(foutput, 'a')
...
def find_kmers(finput, ksize, labelDataset, caracteres):
    ...
    perms(foutput, ksize, caracteres)
    for seq_record in SeqIO.parse(io.StringIO(finput), "fasta"):
        ...
        fileRecord(foutput, name_seq, probabilities, labelDataset)
    return foutput
